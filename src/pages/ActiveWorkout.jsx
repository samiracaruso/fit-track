...
export default function ActiveWorkout() {
  ...
  const [activeSession, setActiveSession] = useState(null); // Separate state for clarity
  ...
  const loadSessions = async () => {
    try {
      ...
      if (user) {
        ...
        if (data && !error) {
          setSessions(data);
          // Sync fresh history to Dexie
          // We use bulkAdd in the background to ensure local matches cloud
          await localDB.history.clear(); 
          await localDB.history.bulkAdd(data.filter(s => s.status === 'completed'));
        }
      }
    } catch (error) {
  ...
  const continueWorkout = (session) => {
    // Navigate to the ACTUAL tracker page
    navigate(`/ActiveSession?day=${session.day_of_week}&resume=true`);
  };
  ...
  if (loading && sessions.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-[#0a0a0a] gap-4">
        ...
        <p className="text-zinc-500 font-bold uppercase text-[10px] tracking-widest">Loading Records...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] pb-8 text-white">
      ...
      <div className="bg-gradient-to-b from-[#1a1a1a] to-[#0a0a0a] px-6 pt-8 pb-6">
        <button onClick={() => navigate('/')} className="mb-4 flex items-center gap-2 text-zinc-500 font-bold uppercase text-xs tracking-widest active:text-cyan-500 transition-colors">
      ...
      {activeSession && (
        <div className="px-6 mt-10">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.2em]">Active Protocol</h2>
          </div>
          <button
            onClick={() => continueWorkout(activeSession)}
            className="w-full bg-zinc-900 border border-cyan-500/50 rounded-[2rem] p-6 flex items-center justify-between group active:bg-zinc-800 transition-colors relative overflow-hidden"
          >
            <div className="text-left relative z-10">
              <p className="font-black text-lg uppercase italic text-cyan-500 leading-tight">
                {activeSession.day_of_week} Session
              </p>
              <p className="text-xs text-zinc-500 font-bold uppercase">
                {activeSession.exercises?.filter(e => e.completed).length || 0} exercises logged
              </p>
            </div>
            <div className="relative z-10 text-cyan-500 text-[10px] font-black uppercase px-4 py-2 rounded-xl border border-cyan-500/30 group-hover:bg-cyan-500 group-hover:text-black transition-all">
              RESUME
            </div>
            {/* Subtle background glow for active item */}
            <div className="absolute top-0 right-0 w-32 h-full bg-cyan-500/5 blur-2xl" />
          </button>
        </div>
      )}
        ...
        <div className="space-y-4">
          {sessions.length === 0 ? (
             <div className="text-center py-16 border-2 border-dashed border-zinc-900 rounded-[2.5rem]">
                ...
                <p className="text-zinc-600 font-bold uppercase text-xs tracking-widest">No history recorded</p>
             </div>
            ...
            <div key={session.id} className="bg-zinc-900/50 border border-zinc-800/50 rounded-[2rem] p-6">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-black/40 p-3 rounded-2xl text-center">
                  ...
                  <p className="text-[9px] font-bold text-zinc-600 uppercase tracking-tighter">Exers</p>
                </div>
                <div className="bg-black/40 p-3 rounded-2xl text-center">
                  <p className="text-orange-500 font-black text-lg">{Math.round(session.total_calories_burned || 0)}</p>
                  <p className="text-[9px] font-bold text-zinc-600 uppercase tracking-tighter">Kcal</p>
                </div>
                <div className="bg-black/40 p-3 rounded-2xl text-center">
                  ...
                  <p className="text-[9px] font-bold text-zinc-600 uppercase tracking-tighter">Mins</p>
                </div>
              </div>
            </div>
          ))}
