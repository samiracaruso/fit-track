...
import { 
  Dumbbell, History, Plus, Calendar as CalendarIcon, 
...
export default function Home() {
  ...
  useEffect(() => {
    async function loadDashboardData() {
      ...
      if (user && localPlans.length === 0 && navigator.onLine) {
        const { data: cloudPlans } = await supabase
          .from('workout_plans').select('*').eq('user_id', user.id);
        if (cloudPlans?.length > 0) await db.plans.bulkPut(cloudPlans);
      }
    }
    loadDashboardData();
  }, []);
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white pb-24">
      <header className="px-6 pt-12 pb-6 flex justify-between items-start">
        <div>
          ...
          <p className="text-zinc-500 text-[10px] font-black uppercase tracking-[0.2em] mt-2">
            Base 44 Signal: <span className="text-green-500 ml-1">● Online</span>
          </p>
        </div>
        <div className="flex gap-2">
          <Button variant="ghost" size="icon" onClick={() => navigate('/Profile')} className="bg-zinc-900 rounded-full border border-white/5">
      ...
      <div className="px-6 grid grid-cols-2 gap-3 mt-4">
        <Button onClick={() => navigate('/ActiveSession')} className="h-32 bg-cyan-500 hover:bg-cyan-400 text-black rounded-[2rem] flex-col gap-2 shadow-[0_0_20px_rgba(6,182,212,0.15)]">
          <Plus size={24} strokeWidth={3} />
          <span className="font-black uppercase italic text-xs">Start Lift</span>
        </Button>
        <Button onClick={() => navigate('/WeeklyPlan')} variant="outline" className="h-32 border-zinc-800 bg-zinc-900/50 rounded-[2rem] flex-col gap-2 text-white hover:bg-zinc-800">
        ...
        <div className="space-y-3">
          {recentHistory.length > 0 ? (
            ...
            <div className="text-center py-12 border-2 border-dashed border-zinc-900 rounded-[2.5rem]">
              <p className="text-zinc-700 text-[10px] font-black uppercase tracking-widest">No moves recorded</p>
            </div>
          )}
        </div>
      </section>
